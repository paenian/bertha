...
module hotend_effector_body(solid=1, height=15){
	if(solid==1){
		...
		for(i=[0:120:359]) rotate([0,0,i]) translate([0,- arm_rad, -.5]) hull(){
			translate([0,-623_rad,0]) scale([1,.5,1]) cylinder(r=arm_mount_sep/2,h=height+1);
...
module hotend_effector(){
	...
	difference(){
		union(){
			difference(){
				union(){
					arm_mounts(solid=1, height=h);
					hotend_effector_body(solid=1, height=h);
				}
				arm_mounts(solid=0, height=h);
				hotend_effector_body(solid=0, height=h);
			}

			rotate([0,0,60]) hotend_tripleclamp(height=h, solid=1);
		}
		
		rotate([0,0,60]) hotend_tripleclamp(height=h, solid=0);
	}
}
...
module wheel_mount(solid=1, rad=6, height=5, eccentric=0){
	if(solid){
		...
		if(eccentric == 1){
			...
			translate([0,0,height-m5_nut_rad]) cylinder(r=m5_nut_rad, h=m5_nut_height+.1, $fn=6);
		}
	}
}
	...
	if(solid){
		minkowski(){
			...
			difference(){
				intersection(){
					rotate([0,-90,0])
					linear_extrude(height=arm_mount_sep+slop/2+h*2+3-m_rad*2, center=true) barbell([0,623_rad-2],[inset,0],623_rad*2-m_rad,623_rad+2-m_rad,18,200);
		...
		for(i=[0:1]) mirror([i,0,0]){
			translate([arm_mount_sep/2+h+slop/2,0,inset]) rotate([0,-90,0]) rotate([0,0,90]) 623_bearing_mount(rad=623_rad+1, height=h, solid=solid);
		}
...
module rail_effector_helper(solid=1, rod_inset = 0, rod_offset = 2, height=10){
	...
	arm_bearings(solid=solid, height=h, inset=rod_inset, offset = rod_offset, base_height=height);


	//wheel mounts
	translate([0,wheel_z_sep/2+rod_offset, 0])
...
module rail_effector(){
	...
	difference(){
		union(){
			...
			translate([pulley_rad,wall*2,0]) wheel_mount(solid=1, rad=7, height=height, eccentric=0);
		...
		//screws to attach belts to
		translate([pulley_rad,wall*2,0]) wheel_mount(solid=0, rad=7, height=height, eccentric=0);
