...
module hotend_effector(clamp_height = 15){
	//local variables
	...
	difference(){
		union(){
			...
			*for(i=[0:120:359]) rotate([0,0,i])
				hull(){
					translate([0,-hotend_attachment_rad,0]) cylinder(r=nut_rad+1, h=clamp_height);
					translate([0,-hotend_attachment_rad+wall,0]) cylinder(r=nut_rad, h=clamp_height);
				}
			
			//center fill
			cylinder(r=wall*2, h=clamp_height);

			//extruder mounts
			for(i=[0:120:359]) rotate([0,0,i])
				translate([0,extruder_rad,0]) extruder_mount(1,clamp_height,0,0);
		}

		//extruder mounts
		for(i=[0:120:359]) rotate([0,0,i])
			translate([0,extruder_rad,0]) extruder_mount(0,clamp_height,0,0);

		//bolt holes
		for(i=[0:120:359]) rotate([0,0,i])
			translate([0,-hotend_attachment_rad,-.05]) cylinder(r=bolt_rad+.125, h=clamp_height+.1);
		//translate([0,0,wall]) cylinder(r=m3bolt_cap_rad, h=h+.1);
		translate([0,0,-.05]) cylinder(r=m3bolt_rad, h=clamp_height+.1);
