...
nut_mount = wall+nut_height;	//thickness behind cone
...
module extruder_mount(solid = 1){
	...
	if(solid){
		...
		//fillet square
		for(i=[0,1]) mirror([i,0,0]) translate([arm_sep/2-cone_height-nut_mount-fillet+.05,0,mount_height-.05]) rotate([90,0,0]) cube([fillet, fillet, wall*2]);
	}else{
		union(){
			//fillet round
			for(i=[0,1]) mirror([i,0,0]) translate([arm_sep/2-cone_height-nut_mount-fillet-.05,.05,mount_height+fillet-.01]) rotate([90,0,0]) cylinder(r=fillet,h=wall*2+3);
