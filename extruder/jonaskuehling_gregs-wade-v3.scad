...
eigthmpt=1024;	//eigth inch pipe thread, for bowden mounting
	...
	difference (){
		union(){
			...
			linear_extrude(height=motor_mount_thickness){
				...
				barbell([base_length+base_thickness/2,
					wade_block_height],motor_hole(0),base_thickness/2,
					nema17_support_d/2,100,60);
				barbell([base_length+base_thickness/2,
					wade_block_height/2],motor_hole(3),base_thickness/2,
					nema17_support_d/2,100,60);
			}
			...
			*translate([-base_leadout,base_thickness/2,0])
			...
			render()
			difference(){
				cube([wade_block_width+block_bevel_r,
			...
			translate([0,0,wade_block_depth/2]) rotate([-90,0,0]) cylinder(r=wade_block_depth/2, h=base_thickness);

			render() extrusion_bracket();
		...
		rotate([-90,0,0]){
			...
			if (in_mask(hotend_mount,eigthmpt))
				eigthmpt_mount_holes();
		}
	}
}
...
module extrusion_bracket(){
	...
	difference(){
		union() {
			cube([base_thickness, wade_block_height+wall, wade_block_depth]);
			translate([0,wade_block_height+wall,wade_block_depth/2]) rotate([0,90,0]) cylinder(r=wade_block_depth/2, h=base_thickness);
			
			translate([-block_bevel_r*2, base_thickness-1,0]) cube([block_bevel_r*2+1, block_bevel_r*2+1, wade_block_depth]);
		}
		//bevel
		translate([-block_bevel_r*2, base_thickness+block_bevel_r*2,-.1]) cylinder(r=block_bevel_r*2,h=wade_block_depth+1,$fn=60);

		//mounting holes
		for(i=[15, wade_block_height+wall]){
			translate([0,i,wade_block_depth/2]) rotate([0,90,0]) cylinder(r=bolt_rad, h=15, center=true);
			translate([-wall-5,i,wade_block_depth/2]) rotate([0,90,0]) cylinder(r=bolt_cap_rad, h=base_thickness+5);
		}
	}
}
...
module motor_mount(){
	linear_extrude(height=motor_mount_thickness){
		...
		barbell (motor_hole(2),motor_hole(3),nema17_support_d/2,
			nema17_support_d/2,20,160);
		barbell (motor_hole(3),motor_hole(0),nema17_support_d/2,
			nema17_support_d/2,20,160);
	}
}
...
module eigthmpt_mount_holes(){
	...
	// Recess in base
	translate([0,0,-1])
	cylinder(r=tap_rad,h=extruder_recess_h+1);

	//flatten the top
	//translate([0,0,extruder_recess_h-1])
	//cylinder(r=tap_rad+.5,h=1);
